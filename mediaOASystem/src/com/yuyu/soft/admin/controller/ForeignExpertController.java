...
//            Font font = POIUtil.getBaseFont(sheet);
    ...
    public void foreign_expert_add_save(HttpServletRequest request, HttpServletResponse response,
        ...
        if(true == rmg.getResult()){
        	User user = new User();
        	user.setMobile(foreignExpert.getMobile());
        	user.setTrue_name(foreignExpert.getChinese_name());
        	user.setBirthday(foreignExpert.getBirthday());
        	Department department = new Department();
        	department.setId(10L);
        	user.setDepartment(department);
        	user.setPassword("888888");
        	user.setDuty(foreignExpert.getDuty());
        	user.setCard_number(foreignExpert.getCard_number());
        	user.setUser_status(3);
        	userService.add_save(user, department.getId(), duty_id, null,null,null);
        }
